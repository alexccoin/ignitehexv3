-- Drop all existing functions that need to be recreated
...
-- Auto Encryption System Improvements for Mars Colony
-- This handles all user data encryption seamlessly without breaking access

-- Function to auto-generate PIN for users based on STR domain
CREATE OR REPLACE FUNCTION public.auto_generate_user_pin(user_id_param UUID)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET 
    ...
    END,
    -- Only clear plaintext if we have encrypted version
    access_token = CASE 
