-- Fix validation triggers to use correct field names - cascade drop dependencies

-- Drop all existing validation triggers explicitly first
...
DROP TRIGGER IF EXISTS ensure_github_token_encrypted ON github_integrations;
DROP TRIGGER IF EXISTS ensure_recovery_words_encrypted ON user_profiles;
