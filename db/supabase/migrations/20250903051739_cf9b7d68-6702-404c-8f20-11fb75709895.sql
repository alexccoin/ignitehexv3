-- Fix ambiguous column reference and create missing calculate_dynamic_apy function

-- First, create the missing calculate_dynamic_apy function
CREATE OR REPLACE FUNCTION public.calculate_dynamic_apy(
  stake_amount NUMERIC,
  stake_duration_months INTEGER,
...
BEGIN
  ...
  WHERE duration_months = stake_duration_months
    AND status = 'active'
  LIMIT 1;
  ...
  amount_multiplier := LEAST(
    (stake_amount - pool_record.min_stake_amount) / 
  ...
  duration_multiplier := 1.0 + (stake_duration_months * 0.02); -- 2% bonus per month
...
-- Fix ambiguous column reference in distribute_enhanced_rewards function
...
BEGIN
  ...
  WHERE token_type = token_type_param
    AND duration_months = duration_months_param  -- Fixed ambiguous reference
    AND status = 'active'
  LIMIT 1;
