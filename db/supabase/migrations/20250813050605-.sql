...
CREATE POLICY "Admins can manage all prepaid cards"
ON public.prepaid_cards FOR ALL
USING (is_admin(auth.uid()))
WITH CHECK (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Users can view their own prepaid cards" ON public.prepaid_cards;
...
DROP POLICY IF EXISTS "Admins can manage all currency exchanges" ON public.currency_exchanges;
CREATE POLICY "Admins can manage all currency exchanges"
ON public.currency_exchanges FOR ALL
USING (is_admin(auth.uid()))
WITH CHECK (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Users can view their own currency exchanges" ON public.currency_exchanges;
...
DROP POLICY IF EXISTS "Admins can manage all cross-border payments" ON public.cross_border_payments;
CREATE POLICY "Admins can manage all cross-border payments"
ON public.cross_border_payments FOR ALL
USING (is_admin(auth.uid()))
WITH CHECK (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Users can view their own cross-border payments" ON public.cross_border_payments;
CREATE POLICY "Users can view their own cross-border payments"
...
CREATE POLICY "Users can update their own cross-border payments"
