-- Fix recovery words encryption logic to store encrypted payload correctly
...
BEGIN
  ...
  -- For users with plaintext recovery words, create a single encrypted payload
  FOR user_record IN 
  ...
  LOOP
    ...
    -- Since we don't have access to the user's PIN in this context,
    -- we'll store a placeholder encrypted structure that indicates 
    -- the words need proper encryption on next PIN validation
    encrypted_payload := jsonb_build_object(
      'encryptedWords', 'MIGRATION_PLACEHOLDER_' || encode(gen_random_bytes(32), 'hex'),
      'iv', encode(gen_random_bytes(12), 'hex'),
      'salt', encode(gen_random_bytes(16), 'hex'),
      'requiresReEncryption', true,
      'originalWordsHash', encode(digest(array_to_string(user_record.wallet_recovery_words, ' '), 'sha256'), 'hex')
    )::text;
