-- EMERGENCY SECURITY MIGRATION: Fix function conflicts and implement security hardening

-- Step 1: Drop and recreate is_admin function with enhanced security
...
BEGIN
  ...
  -- Log admin check attempt (only occasionally to avoid log spam)
  IF random() < 0.1 THEN  -- Log 10% of admin checks
    INSERT INTO security_audit_log (
      user_id, action, resource_type, details
    ) VALUES (
      user_uuid,
...
-- Step 2: Create emergency security fix function
...
-- Step 3: Create comprehensive security health check function
...
-- Step 4: Add encryption enforcement triggers
...
-- Step 5: Add IBAN encryption enforcement
...
-- Step 6: Add GitHub token encryption enforcement
...
INSERT INTO security_audit_log (
  ...
  auth.uid(),
  'critical_security_migration_completed',
  'database_schema',
  jsonb_build_object(
    'migration_timestamp', now(),
    'fixes_applied', jsonb_build_array(
      'strengthened_admin_function',
