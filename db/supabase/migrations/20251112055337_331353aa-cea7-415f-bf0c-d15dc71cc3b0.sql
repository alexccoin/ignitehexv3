-- Update get_all_users_admin function to explicitly handle more users
-- Supabase PostgREST has a default limit, but RPC functions should return all rows

-- Drop and recreate with explicit handling for large result sets
...
BEGIN
  ...
  -- Return all users (no limit) with additional location data
  RETURN QUERY
  SELECT 
    up.id,
    au.id as user_id,
  ...
  -- No LIMIT - return all users up to 10,000+
END;
$function$;